...
export class KanbanView extends BasesView {
  ...
  private renderCard(
    ...
    cardEl.toggleClass(
      ...
      this.selectedPaths.has(filePath),
    );
  ...
  private setupCardDragBehavior(cardEl: HTMLElement): void {
    ...
    cardEl.addEventListener("mouseup", () => {
      if (this.draggingSourcePath === null) {
        cardEl.draggable = false;
      }
    });
  ...
  private getColumnEl(columnKey: string): HTMLElement | null {
    const columns = this.rootEl.querySelectorAll<HTMLElement>(
      ".bases-kanban-column",
    );
...
function hasConfiguredGroupBy(groups: Array<{ key?: unknown }>): boolean {
  return (
    groups.length > 1 ||
    (groups.length === 1 &&
...
function isSameGroupValue(
  currentValue: unknown,
  targetValue: string | null,
): boolean {
  if (
    currentValue instanceof NullValue ||
    currentValue === null ||
