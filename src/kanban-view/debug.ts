...
const shouldLogRender = (): boolean => {
	if (typeof window !== "undefined" && window.__KANBAN_DEBUG_RENDERS__ === true) {
		return true;
	}
...
const shouldLogScroll = (): boolean => {
	if (typeof window !== "undefined" && window.__KANBAN_DEBUG_SCROLL__ === true) {
		return true;
	}
...
export function measureTime<T>(
	...
	if (duration >= thresholdMs) {
		const timestamp = new Date().toISOString().split("T")[1]?.split(".")[0];
		console.log(
			`[${timestamp}] [PERF] ${label}: ${duration.toFixed(2)}ms`,
		);
	}
